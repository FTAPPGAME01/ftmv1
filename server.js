...
io.on('connection', (socket) => {
    ...
    socket.on('updateState', (updatedState) => {
        ...
        socket.on('dailyReset', () => {
            gameState.resetCount = 0; // Reset the count to 0 at the start of each day
            io.emit('updateResetCount', gameState.resetCount);
        });
    
        // Add an event to get the current reset count
        socket.on('getResetCount', () => {
            socket.emit('updateResetCount', gameState.resetCount);
        });
...
const resetGame = () => {
    ...
    gameState.resetCount++; // Increment the reset count
